...
def merge_last_result():
    reader = csv.reader(open("./result/merge_conv_result.csv", "r", encoding="utf-8"))
    df = pd.read_csv(open("./data/round2/test_correlation.csv", "r", encoding="utf-8"))

    with open("./result/merge_result_28.csv", "w", encoding="utf-8", newline="") as file:
        ...
        for index, row in enumerate(reader):
            if index == 0:
                ...
                temp = np.mean(last[index-1], axis=0)*0.2 + float(row[1])*0.8
